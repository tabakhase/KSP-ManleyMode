...
namespace ManleyMode
{
	...
	public class ManleyModule : PartModule
	{
		...
		public bool forceIVA { get; set; }//= true;
		public bool bailingOut { get; set; }// = false;

		protected static ManleyModule gui_controller;

		public static ManleyModule gui
		{
		...
		{
			Debug.Log("[ManleyModule] OnAwake");
		...
		void onHideUI()
		{
			if(!ManleyModuleSettings.Instance.ShowInNoUiMode)
				guiEnabled = false;
		}
	    ...
	    {
			Debug.Log("[ManleyModule] OnStart");
			try
			{
				//if (IsPrimary)
				//{
		...
		void OnDestroy()
		{
			Debug.Log("[ManleyModule] OnDestroy");
		...
		{
			Debug.Log(String.Format("[ManleyModule] onVesselChange -> {0}", v.name));
			foreach (Part p in v.Parts) {
				ManleyModule manleyAntenna = p.GetComponent<ManleyModule>();
				if (manleyAntenna != null) {
					Debug.Log("[ManleyModule] onVesselChange - Antenna found! arming ManleyModule...");
					// TODO
				}
			}
		}
				...
				if (IsPrimary)
				{
					...
					if (forceIVA) {
						if (CameraManager.Instance.currentCameraMode != CameraManager.CameraMode.IVA) {
							...
							//Debug.Log("[ManleyModule] OnUpdate - forceIVA -> CameraManager SetCameraIVA");
							CameraManager.Instance.SetCameraIVA ();
						}

						if (MapView.MapIsEnabled) {
							//Debug.Log("[ManleyModule] OnUpdate - forceIVA ->  MapView MapIsEnabled");
							MapView.ExitMapView();
						}


						if (PlanetariumCamera.fetch.enabled) {
							//Debug.Log("[ManleyModule] OnUpdate - forceIVA ->  PlanetariumCamera AbortExternalControl");
						}
					}
				}
			}
	    ...
	    public void requestGUI(int windowID)
	    {
	        ...
	        if (GUILayout.Button("Go on EVA"))
			{
	            if (HighLogic.CurrentGame.Parameters.Flight.CanEVA)
	            {
					...
					Debug.Log("[ManleyModule] requestGUI - EVA is " + crew.crewMemberName);
					forceIVA = false;
					bailingOut = true;
	                if (crew != null && FlightEVA.SpawnEVA(crew))
					{
						...
						Debug.Log("[ManleyModule] requestGUI - EVA BAILE OUT! -done");
	                }
	            }
	        }
	        GUILayout.EndVertical();
		...
		private double getKspDistance()
		{
			...
			if (kerbin.name != "Kerbin") { Debug.Log("kerbin.name != Kerbin - got " + kerbin.name + " instead"); return 0; }
